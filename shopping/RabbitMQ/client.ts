    ...
    async initialize()
    {
        ...
        else
        {
            try 
            {
                ...
                const {queue : replyQueueName} = await this.consumerChannel.assertQueue("",{exclusive:true}) // By not giving it a name, RabbitMQ assigns the queue a unique random unique name which we store in 'replyQueueName'. Exclusive:true means that the queue is deleted if RabbitMQ connection dies.
                
                this.eventEmitter = new EventEmitter();
                this.producer = new Producer(this.consumerChannel,replyQueueName,this.eventEmitter);
